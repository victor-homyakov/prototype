...
var Form = {
   ...
   *  Example usage:
   *
   *      Form.reset('contact') // returns form, chaining is possible
   *
   *      // equivalent:
   *      $('contact').reset(); // returns nothing
   *
...
Form.Element.Methods = {
  ...
  **/
   ...
   *
   *  For `checkbox` and `radio` form controls sets `checked` state to the `value`.
   *  For `select` form control marks the option with the value `value` as selected.
   *  For other form controls sets `value` to be the value of the form control.
   *  Returns the element.
   *
   *  ##### Example
   *
   *        $$('input[type="radio"]').invoke('setValue', true);
   *        $('select1').setValue('option1');
